var entityManager = (function () {
  ...
  class UI {
    ...
    <td><a href="#" class="btn btn-danger btn-sm text-white font-weight-normal"data-action="edit" data-id="${key}">Editar</a></td>
    <td><a href="#" class="btn btn-danger btn-sm text-white font-weight-normal" data-action="delete" data-id="${key}">Eliminar</a></td>
  `;
  ...
  function init(param) {
      ...
      messages.deleted = `Promocion Eliminada`;
      messages.added = `Promocion Agregada`;
      messages.edited = `Promocion Editada`;
      ...
      //---- SET THE TITLE
      let nombre_key_target = `[data-id=nombre]`;
      let nombre_target = main_container.querySelector(nombre_key_target);
      if (nombre_target == undefined) {
          console.error('DOM target not found for ID ' + nombre_key_target);
      } else {
          nombre_target.innerHTML = nombre_entity;
      }
      ...
      if (form_container == undefined) {
          ...
          form_container.innerHTML = `${columns}
              ...
              <input type="submit" value="Submit creation" data-id="submit-button" class="btn btn-danger btn-block" >
              <input type="button" value="Cancelar" data-id="cancel-button" class="btn btn-danger btn-block" >`;
  ...
  window.addEventListener("DOMContentLoaded", (event) => {
      ...
      let mockup = [
          {
              nombre: 'Descuento en Locales de Cafe',
              descripcion: '50% de descuento',
              Codigo: 'PRO089'
          },
          {
              nombre: '2X1 en KFC',
              descripcion: '50% en Hamburguesas',
              Codigo: 'PRO893'
          },
      ];
